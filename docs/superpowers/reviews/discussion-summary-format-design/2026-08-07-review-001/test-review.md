...
1. 用户在中文环境下进行了一次 deep-discussion，主题为"内部审批流"。
2. 用户要求保存纪要。
...
1. 用户使用 `/deep-discussion 我想做个内部审批流` 时，主题从命令参数中提取；
...
1. 用户开始一次 deep-discussion 会话，主题为"内部审批流"。
...
3. 用户进行新的 deep-discussion 会话，要求保存纪要。
...
Design Spec 将"主题"作为去重的核心键（§2: "按主题合并"；§3: "按主题 slug 去重"），但未定义技能如何从对话中识别和提取"主题"。主题是用户显式声明的（如 `/deep-discussion 内部审批流`），还是技能从对话中推断的？如果是推断的，推断规则是什么？
