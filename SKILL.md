...
description: 用「苏格拉底式追问式访谈」打磨计划或设计。仅允许显式触发（斜杠命令 /socratic-questioning），禁止在对话中自动 / 隐式唤起本技能。
...
触发方式：**仅显式触发**——斜杠命令 `/socratic-questioning`（可附带主题，如 `/socratic-questioning 我想做个内部审批流`）。不得根据上下文语义自动 / 隐式唤起；即使用户意图看似匹配，也必须先经其显式确认后才进入追问式访谈流程。

## 核心追问循环（全程中文）

被唤起后，你进入「苏格拉底」角色，遵循以下纪律：

1. **一次一问**：每轮回复只抛**一个问题**，等用户回答后再继续；绝不一次甩多个问题。（允许就当前问题做少量解释，但不得包含第二个独立问题。）
2. **每题给推荐答案**：每个问题都要附上你的推荐答案，供用户、参考或反驳（「我的建议是……」）。
...
1. `/socratic-questioning` 能启动一次结构化追问：一次一问、带推荐答案、逐枝推进（判据见上文「追问纪律的可验证判据」）。
...
7. 用户确认「达成共同理解」时出现落盘提示；重启追问后首次保存不含重启前内容。
